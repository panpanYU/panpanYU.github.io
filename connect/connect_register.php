...
/*
   ...
   $Id: connect_register.php 139 2011-09-22 06:22:18Z liuwenxue $
*/
...
$uinlimit = $connect['register_uinlimit'] && $db->result_first("SELECT COUNT(DISTINCT uid) FROM {$tablepre}connect_memberbindlog WHERE uin='$conopenid' AND type='1'") >= $connect['register_uinlimit'];
...
if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	include template('qqconnect:register');

} else {
	if ($uinlimit) {
		showmessage('qqconnect:connect_register_uinlimit', '', array('limit' => $connect['register_uinlimit']));
	}
	...
	$db->query("INSERT INTO {$tablepre}members (uid, username, password, secques, adminid, groupid, regip, regdate, lastvisit, lastactivity, posts, credits, extcredits1, extcredits2, extcredits3, extcredits4, extcredits5, extcredits6, extcredits7, extcredits8, email, showemail, timeoffset, pmsound, invisible, newsletter, conisbind)
		VALUES ('$uid', '$username', '$password', '$secques', '0', '$groupinfo[groupid]', '$onlineip', '$timestamp', '$timestamp', '$timestamp', '0', $initcredits, '$email', '0', '9999', '1', '0', '1', '1')");

	$db->query("INSERT INTO {$tablepre}member_connect (uid, conuin, conuinsecret, conopenid, conisregister) VALUES ('$uid', '$conuin', '$conuinsecret', '$conopenid', '1')");
	...
	dsetcookie('stats_qc_reg', 1, 86400, 0, false);
	$inshowmessage = 1;
	$db->query("INSERT INTO {$tablepre}connect_memberbindlog (uid, uin, type, dateline) VALUES ('$uid', '$conopenid', '1', '$timestamp')");
