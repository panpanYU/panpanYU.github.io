...
/*
	...
	$Id: MyBase.php 133 2011-09-22 03:20:06Z yexinhao $
*/
...
class MyBase {
	...
	function _space2user($member) {
		...
		$user = array(
			...
			'homepagePrivacy'	=> 'public',//note $privacy['view']['index'] == 1 ? 'friends' : ($privacy['view']['index'] == 2 ? 'me' : 'public'),
			'profilePrivacy'	=> 'public',//note $privacy['view']['profile'] == 1 ? 'friends' : ($privacy['view']['profile'] == 2 ? 'me' : 'public'),
			'friendListPrivacy'	=> 'public',//note $privacy['view']['friend'] == 1 ? 'friends' : ($privacy['view']['friend'] == 2 ? 'me' : 'public')
		);
		return $user;
	}
...
class my{
	...
	function parseRequest() {
		...
		$siteuniqueid = $GLOBALS['db']->result_first($uniqueidsql);
		if (empty($siteuniqueid)) {
			$errCode = '11';
			$errMessage = 'Client SiteKey NOT Exists';
		} elseif (empty($_DCACHE['settings']['my_sitekey'])) {
			$errCode = '12';
	...
	function callback($module, $method, $params) {
		...
		@include_once DISCUZ_ROOT.'./api/manyou/class/'.ucfirst($module).'.php';
